...
def wait_udp_ready(timeout: float = 5.0) -> bool:
    return _ready.wait(timeout)


def get_udp_bind_error():
    return _bind_error
            ...
            with transfers_lock:
                ...
                if key in active_transfers:
                    ...
                    if transfer["received"] >= transfer["filesize"]:
                        ...
                        safe_log(
                            f"SUCCESS: Received {transfer['filename']} ({transfer['received']} bytes) from {addr}"
                        )
